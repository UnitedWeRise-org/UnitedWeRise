...
class ExternalCandidateService {
     ...
     */
    static async getCandidatesForAddress(address) {
        try {
            ...
            const candidatesWithOffices = await prisma_1.prisma.candidate.findMany({
                where: {
                    OR: [
            ...
            for (const candidate of candidatesWithOffices) {
                ...
                const normalizedOfficeKey = this.normalizeOfficeTitle(candidate.office.title, candidate.office.level, candidate.office.district);
            ...
            logger_1.default.error('Failed to get candidates for address:', error);
            return [];
        }
    }
    /**
     ...
     */
    static normalizeOfficeTitle(title, level, district) {
