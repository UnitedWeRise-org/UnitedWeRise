...
class PushNotificationService {
    ...
    initializeProvider() {
        try {
            ...
            if (!keyId || !teamId || !bundleId || !keyBase64) {
                logger_1.logger.warn('Push notification service not configured. Set APNS_KEY_ID, APNS_TEAM_ID, APNS_BUNDLE_ID, APNS_KEY.');
                return;
            }
            const signingKey = Buffer.from(keyBase64, 'base64');
            this.client = new apns2_1.ApnsClient({
                team: teamId,
                keyId: keyId,
                signingKey: signingKey,
                defaultTopic: bundleId,
                host: (0, environment_1.isProduction)() ? apns2_1.Host.production : apns2_1.Host.development,
                requestTimeout: 5000,
                keepAlive: true,
            });
            logger_1.logger.info({
                environment: (0, environment_1.isProduction)() ? 'production' : 'sandbox',
                bundleId
            }, 'Push notification service initialized');
        }
        catch (error) {
            logger_1.logger.error({ error }, 'Failed to initialize push notification service');
        }
    }
    /**
    ...
    async sendToUser(userId, payload) {
        ...
        try {
            ...
            await Promise.allSettled(devices.map(async (device) => {
                try {
                    const notification = new apns2_1.Notification(device.deviceToken, {
                    ...
                    const apnsError = error;
                    const reason = apnsError?.reason;
                    if (reason === apns2_1.Errors.badDeviceToken ||
                        reason === apns2_1.Errors.unregistered ||
                        reason === apns2_1.Errors.deviceTokenNotForTopic) {
                        invalidTokenIds.push(device.id);
                        logger_1.logger.info({ deviceId: device.id, reason }, 'Invalid device token, marking for cleanup');
                    }
                    else {
                        logger_1.logger.error({ error, deviceId: device.id }, 'APNs send error');
                    }
                }
            }));
            if (invalidTokenIds.length > 0) {
                await prisma_1.prisma.deviceToken.deleteMany({
                    where: { id: { in: invalidTokenIds } }
                });
                logger_1.logger.info({ count: invalidTokenIds.length, userId }, 'Cleaned up invalid device tokens');
            }
            ...
            logger_1.logger.error({ error, userId }, 'Failed to send push notification');
            return false;
        }
    }
    /**
     ...
     * );
     */
    async sendMessagePush(recipientId, senderName, messageContent, conversationId, messageType = 'USER_USER') {
     ...
     * @returns True if push was delivered to at least one device
     */
    async sendEncryptedMessagePush(recipientId, senderName, encryptedContent, conversationId, senderPublicKeyId, messageType = 'USER_USER') {
        return this.sendToUser(recipientId, {
            title: senderName,
            body: 'New message',
            threadId: conversationId,
