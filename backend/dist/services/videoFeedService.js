...
exports.VideoFeedService = void 0;
...
class VideoFeedService {
     ...
     */
    static async generateFeed(options) {
        const { userId, feedType = 'for-you', limit = 15, excludeIds = [], customWeights } = options;
     ...
     */
    static async generateForYouFeed(userId, limit, excludeIds, weights) {
        const cutoffDate = new Date(Date.now() - CANDIDATE_WINDOW_DAYS * 24 * 60 * 60 * 1000);
        // Get candidate videos
        const candidates = await prisma_js_1.prisma.video.findMany({
            where: {
                videoType: 'REEL',
        ...
        let userHashtags = new Set();
        if (userId) {
            const [follows, likedVideos] = await Promise.all([
                prisma_js_1.prisma.follow.findMany({
                    where: { followerId: userId },
                    select: { followingId: true }
                }),
                prisma_js_1.prisma.videoLike.findMany({
                    where: { userId },
        ...
        const scored = candidates.map(video => {
            ...
            const finalScore = (recencyScore * weights.recency) +
     ...
     */
    static async generateFollowingFeed(userId, limit, excludeIds) {
        ...
        const [follows, subscriptions, friendships] = await Promise.all([
            ...
            prisma_js_1.prisma.subscription.findMany({
                where: { subscriberId: userId },
                select: { subscribedId: true }
            }),
            prisma_js_1.prisma.friendship.findMany({
                where: {
                    OR: [
     ...
     */
    static async generateTrendingFeed(limit, excludeIds) {
     ...
     */
    static weightedSample(scored, count) {
...
exports.VideoFeedService = VideoFeedService;
//# sourceMappingURL=videoFeedService.js.map