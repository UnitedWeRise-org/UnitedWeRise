...
class EmailService {
    ...
    generateEmailVerificationTemplate(email, verifyToken, firstName) {
    ...
    generatePasswordResetTemplate(email, resetToken, firstName) {
    ...
    generateWelcomeTemplate(email, firstName) {
        const name = firstName || 'there';
        return {
            to: email,
    ...
    generateWarningTemplate(email, reason, severity, firstName) {
...
The United We Rise Moderation Team`,
    ...
    generateSuspensionTemplate(email, reason, type, endsAt, firstName) {
        const name = firstName ? firstName : email.split('@')[0];
...
The United We Rise Moderation Team`,
    ...
    generateReportUpdateTemplate(email, reportId, action, notes, firstName) {
        const name = firstName ? firstName : email.split('@')[0];
        return {
            to: email,
...
The United We Rise Moderation Team`,
    ...
    generateWaiverApprovalTemplate(email, candidateName, officeLevel, finalFee, firstName) {
    ...
    generateWaiverDenialTemplate(email, candidateName, officeLevel, originalFee, denialReason, firstName) {
        const name = firstName || candidateName.split(' ')[0];
        return {
            to: email,
    ...
    generateCandidateApprovalTemplate(email, candidateName, officeTitle, officeLevelName, firstName) {
        const name = firstName || candidateName.split(' ')[0];
        return {
            to: email,
    ...
    generateCandidateRejectionTemplate(email, candidateName, officeTitle, rejectionReason, notes, firstName) {
        const name = firstName || candidateName.split(' ')[0];
        return {
            to: email,
    ...
    generateCandidateStatusChangeTemplate(email, candidateName, oldStatus, newStatus, reason, notes, firstName) {
        const name = firstName || candidateName.split(' ')[0];
        const statusColors = {
            'ACTIVE': '#28a745',
            'SUSPENDED': '#ffc107',
    ...
    generateAdminMessageTemplate(email, candidateName, subject, messagePreview, messageType, priority, firstName) {
        const name = firstName || candidateName.split(' ')[0];
