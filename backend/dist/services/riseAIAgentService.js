...
class RiseAIAgentService {
     ...
     */
    static async analyzeContent(content, interactionId) {
        try {
            ...
            const relatedArguments = await argumentLedgerService_1.ArgumentLedgerService.findSimilarArguments(embedding, MAX_CONTEXT_ARGUMENTS);
            // Detect fallacies
            ...
            try {
                ...
                logger_1.logger.warn({ error }, 'AI analysis failed, using heuristic analysis');
                aiAnalysis = null;
            }
            ...
            const relatedWithStance = relatedArguments.map(arg => ({
                ...
                similarity: arg.similarity,
                supportOrRefute: this.determineStance(content, arg.content)
            }));
            // Generate recommendation
            const recommendation = this.generateRecommendation(entropyScore, logicalValidity, evidenceQuality, fallaciesFound, ethicalConcerns);
            // Generate summary
            const summary = aiAnalysis?.summary ?? this.generateHeuristicSummary(content, entropyScore, fallaciesFound, ethicalConcerns);
            return {
                entropyScore,
                logicalValidity,
                evidenceQuality,
            ...
            logger_1.logger.error({ error, interactionId }, 'Analysis failed');
            throw error;
        }
    }
    /**
     ...
     */
    static async processInteraction(interactionId) {
        try {
            const interaction = await riseAIMentionService_1.RiseAIMentionService.getInteraction(interactionId);
            if (!interaction) {
            ...
            await this.extractAndStoreArguments(interaction.targetContent, interaction.triggerPostId, interaction.triggerUserId, analysis);
            logger_1.logger.info({ interactionId, responseCommentId: replyComment.id }, 'Completed RiseAI interaction with reply');
            return {
                success: true,
                responseContent,
                responseCommentId: replyComment.id
            ...
            logger_1.logger.error({ error, interactionId }, 'Failed to process interaction');
            await riseAIMentionService_1.RiseAIMentionService.updateInteraction(interactionId, {
                status: 'failed'
            });
            return {
     ...
     */
    static async createReplyComment(params) {
        ...
        logger_1.logger.info({ commentId: comment.id, postId: params.postId }, 'Created RiseAI reply comment');
        return comment;
    }
    /**
     * Detect logical fallacies in content
     */
    static detectFallacies(content) {
        const detected = [];
     ...
     */
    static assessEthicalFramework(content) {
        const concerns = [];
     ...
     */
    static calculateEntropyScore(content, analysis, ethicalConcerns) {
     ...
     */
    static determineStance(content, relatedContent) {
        ...
        const hasNegation = negationPatterns.some(p => relatedContent.toLowerCase().includes(p + ' ' + content.toLowerCase().split(' ')[0]));
        if (hasNegation)
            return 'refute';
        // Check overlap
        let overlap = 0;
        for (const word of contentWords) {
            if (relatedWords.has(word) && word.length > 3)
                overlap++;
        }
     ...
     */
    static generateRecommendation(entropyScore, logicalValidity, evidenceQuality, fallacies, ethicalConcerns) {
     ...
     */
    static formatResponse(analysis) {
        const lines = [];
        // Entropy assessment
     ...
     */
    static generateHeuristicSummary(content, entropyScore, fallacies, ethicalConcerns) {
        const parts = [];
        if (entropyScore >= 7) {
     ...
     */
    static async performAIAnalysis(content, argumentContext, detectedFallacies) {
        const contextStr = argumentContext.length > 0
            ? `\n\nRelated arguments from the discourse:\n${argumentContext.map(a => `- (confidence: ${a.confidence.toFixed(2)}) ${a.content}`).join('\n')}`
            : '';
...
Respond in JSON format:
{
  ...
  "summary": "Brief 1-2 sentence summary of the argument's strengths and weaknesses"
        ...
        try {
            ...
            const response = await azureOpenAIService_1.azureOpenAI.generateCompletion(fullPrompt, {
                maxTokens: 300,
                temperature: 0.3
            });
            // Parse JSON response
...
Please provide a thoughtful, balanced response to the user's message. If they asked a question, answer it directly. If they made an argument, analyze its strengths and weaknesses constructively.`;
     ...
     */
    static async extractAndStoreArguments(content, postId, userId, analysis) {
        try {
            // Check if similar argument already exists
            const embedding = await embeddingService_1.EmbeddingService.generateEmbedding(content);
            const similar = await argumentLedgerService_1.ArgumentLedgerService.findSimilarArguments(embedding, 1);
            ...
            await argumentLedgerService_1.ArgumentLedgerService.createArgument({
            ...
            logger_1.logger.warn({ error }, 'Failed to extract and store argument');
        }
    }
}
exports.RiseAIAgentService = RiseAIAgentService;
//# sourceMappingURL=riseAIAgentService.js.map