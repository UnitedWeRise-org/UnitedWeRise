...
class CandidateReportService {
     ...
     */
    static calculateGeographicWeight(reporterInfo, candidateInfo) {
     ...
     */
    static async assessReportUrgency(reason, description, candidateName, officeTitle) {
        try {
            const prompt = `
      ...
      `;
            const content = await azureOpenAIService_1.azureOpenAI.generateCompletion(prompt, {
                temperature: 0.3,
                maxTokens: 200,
     ...
     */
    static async submitCandidateReport(reporterId, candidateId, reason, description) {
        ...
        const urgencyAssessment = await this.assessReportUrgency(reason, description, candidate.name, candidate.office.title);
     ...
     */
    static async getUserDistrict(user) {
        if (!user?.zipCode)
            return '';
     ...
     * Check for brigading patterns
     */
    static async checkBrigadingPatterns(candidateId) {
        ...
        if (maxReportsPerHour > 10 || lowWeightPercentage > 0.5) {
            ...
            await prisma_1.prisma.contentFlag.create({
                data: {
     ...
     */
    static async getCandidatesDueForVerification() {
     ...
     */
    static async requestVerificationDocuments(candidateId, requestedBy, documentTypes) {
        ...
        const documentRequests = await Promise.all(documentTypes.map(type => prisma_1.prisma.candidateVerificationDocument.create({
            data: {
                candidateId,
                documentType: type,
...
exports.CandidateReportService = CandidateReportService;
exports.candidateReportService = new CandidateReportService();
//# sourceMappingURL=candidateReportService.js.map