 ...
 */
function getSignalWeights(createdAt, interactionCount) {
...
class UserInterestService {
    ...
    static async buildProfile(userId) {
        ...
        try {
            ...
            const dynamicWeights = getSignalWeights(user?.createdAt || new Date(), interactionCount);
            const aggregateVector = this.computeAggregateVector(likedPostEmbeddings, ownPostEmbeddings, user?.embedding || [], dynamicWeights);
            const profile = {
                userId,
                relationships,
                subscribedIds,
     ...
     */
    static computeAggregateVector(likedEmbeddings, ownEmbeddings, userEmbedding, weights = DEFAULT_SIGNAL_WEIGHTS) {
        const allEmbeddings = [];
