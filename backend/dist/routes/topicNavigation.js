...
router.get('/trending', async (req, res) => {
    try {
        const { timeframe = 24, minPosts = 5, maxTopics = 15 } = req.query;
        const trendingTopics = await topicDiscoveryService_1.default.discoverTrendingTopics(Number(timeframe), Number(minPosts), Number(maxTopics));
        res.json({
            success: true,
        ...
        logger_1.default.error('Trending topics discovery failed:', error);
        res.status(500).json({
            success: false,
 ...
 */
router.post('/enter/:topicId', auth_1.requireAuth, async (req, res) => {
    try {
        const { topicId } = req.params;
        const { limit = 30 } = req.body;
        const userId = req.user.id;
        const topicSession = await topicDiscoveryService_1.default.enterTopicMode(topicId, userId, Number(limit));
 ...
 */
router.post('/exit', auth_1.requireAuth, async (req, res) => {
    try {
        const userId = req.user.id;
        ...
        const algorithmFeed = await topicDiscoveryService_1.default.exitTopicMode(userId, navigationState);
...
router.get('/:topicId/posts', async (req, res) => {
    try {
        ...
        logger_1.default.error('Failed to get topic posts:', error);
        res.status(500).json({
            success: false,
...
router.get('/current', auth_1.requireAuth, async (req, res) => {
    try {
        ...
        logger_1.default.error('Failed to get navigation state:', error);
        res.status(500).json({
            success: false,
 ...
 */
router.post('/:topicId/post', auth_1.requireAuth, async (req, res) => {
    try {
        const { topicId } = req.params;
        ...
        logger_1.default.error('Failed to create topic post:', error);
        res.status(500).json({
            success: false,
...
//# sourceMappingURL=topicNavigation.js.map