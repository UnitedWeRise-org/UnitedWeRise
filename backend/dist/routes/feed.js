...
router.get('/', auth_1.requireAuth, async (req, res) => {
    try {
        ...
        logger_1.logger.error({ error, userId: req.user?.id }, 'Feed error');
...
router.get('/following', auth_1.requireAuth, async (req, res) => {
    try {
        ...
        logger_1.logger.error({ error, userId: req.user?.id }, 'Following feed error');
        res.status(500).json({ error: 'Internal server error' });
    }
});
/**
...
router.get('/filters', auth_1.requireAuth, async (req, res) => {
    try {
        ...
        logger_1.logger.error({ error, userId: req.user?.id }, 'Get feed filters error');
        res.status(500).json({
            success: false,
...
router.post('/filters', auth_1.requireAuth, async (req, res) => {
    try {
        ...
        logger_1.logger.error({ error, userId: req.user?.id }, 'Create feed filter error');
        res.status(500).json({
            success: false,
...
router.get('/public', async (req, res) => {
    try {
        ...
        logger_1.logger.error({ error }, 'Public feed error');
        res.status(500).json({
            success: false,
 ...
 *         description: Internal server error
 */
router.get('/slot-roll', auth_1.requireAuth, async (req, res) => {
    try {
        const userId = req.user.id;
        const limit = Math.min(parseInt(req.query.limit?.toString() || '15'), 50);
        // Parse excludeIds for infinite scroll pagination
        ...
        logger_1.logger.error({ error, userId: req.user?.id }, 'Slot-roll feed error');
        res.status(500).json({
            success: false,
