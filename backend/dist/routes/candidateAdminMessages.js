...
router.get('/admin-messages', auth_1.requireAuth, async (req, res) => {
    try {
        ...
        const unreadAdminCount = await prisma_1.prisma.candidateAdminMessage.count({
            where: {
...
router.post('/admin-messages', auth_1.requireAuth, async (req, res) => {
    try {
...
router.get('/admin-messages/unread-count', auth_1.requireAuth, async (req, res) => {
    try {
        // Find candidate profile for current user
        const candidate = await prisma_1.prisma.candidate.findUnique({
            where: { userId: req.user.id },
...
//# sourceMappingURL=candidateAdminMessages.js.map