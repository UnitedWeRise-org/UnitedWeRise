...
router.get('/districts/lookup', async (req, res) => {
    try {
        ...
        const result = await districtIdentificationService_1.DistrictIdentificationService.identifyDistricts(addressComponents, forceRefresh === 'true');
        res.json(result);
    }
    catch (error) {
...
router.post('/districts/missing-offices', async (req, res) => {
    try {
        ...
        const missingOffices = await districtIdentificationService_1.DistrictIdentificationService.findMissingOffices(districtIds);
        res.json({
            missingOffices,
...
router.post('/districts', auth_1.requireAuth, async (req, res) => {
    try {
        ...
        const { name, type, level, identifier, state, county, municipality, coordinates, population, externalId, dataSource, parentDistrict } = req.body;
        // Validation
...
router.post('/districts/:districtId/offices', auth_1.requireAuth, async (req, res) => {
    try {
        const userId = req.user.id;
        ...
        const { title, level, termLength, electionCycle, nextElection, salary, hasTermLimits, maxTerms, currentOfficeholder, holderStartDate, holderEndDate, holderParty, holderContactInfo, holderPhotoUrl, sourceType, sourceUrl, sourceNotes } = req.body;
        // Validation
        if (!title || !level) {
            return res.status(400).json({
...
router.post('/offices/:officeId/officials', auth_1.requireAuth, async (req, res) => {
    try {
        const userId = req.user.id;
        ...
        const { name, party, contactInfo, photoUrl, website, socialMedia, biography, keyAccomplishments, sourceType, sourceUrl, sourceNotes } = req.body;
        // Validation
        if (!name) {
            return res.status(400).json({
...
router.post('/officials/:officialId/vote', auth_1.requireAuth, async (req, res) => {
    try {
        const userId = req.user.id;
...
router.post('/districts/:districtId/conflicts', auth_1.requireAuth, async (req, res) => {
    try {
        const userId = req.user.id;
        const { districtId } = req.params;
        // Rate limiting
