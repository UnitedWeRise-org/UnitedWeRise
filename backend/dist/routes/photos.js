...
const multerDebugWrapper = (req, res, next) => {
 ...
 */
router.post('/upload/sas-token', uploadLimiter, auth_1.requireAuth, async (req, res) => {
    try {
        const { user } = req;
        ...
        await photoService_1.PhotoService.validateStorageLimit(user.id, fileSize);
        // Generate SAS token
        const tokenData = await sasTokenService_1.SASTokenService.generateUploadToken({
            userId: user.id,
            photoType: photoType,
            filename,
            mimeType,
 ...
 */
router.post('/upload', uploadLimiter, auth_1.requireAuth, multerDebugWrapper, async (req, res) => {
    try {
...
router.post('/upload/confirm', uploadLimiter, auth_1.requireAuth, async (req, res) => {
    try {
        ...
        await photoService_1.PhotoService.validateUserPermissions(user.id, candidateId);
        // Create database record using PhotoService method
        const photoRecord = await photoService_1.PhotoService.createPhotoRecordFromBlob({
            userId: user.id,
            blobName,
