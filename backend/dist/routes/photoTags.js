...
const photoTaggingService_1 = require("../services/photoTaggingService");
const client_1 = require("@prisma/client");
const express_rate_limit_1 = __importDefault(require("express-rate-limit"));
const router = express_1.default.Router();
// Rate limiting for tagging actions
const taggingLimiter = (0, express_rate_limit_1.default)({
 ...
 */
router.post('/', auth_1.requireAuth, taggingLimiter, async (req, res) => {
    try {
        const { user } = req;
        ...
        const tag = await photoTaggingService_1.PhotoTaggingService.createTag({
            photoId,
            taggedById: user.id,
            taggedId,
            x: parseFloat(x),
 ...
 *       - bearerAuth: []
 */
router.post('/:tagId/respond', auth_1.requireAuth, async (req, res) => {
    try {
        const { user } = req;
        ...
        const tag = await photoTaggingService_1.PhotoTaggingService.respondToTag({
            tagId,
            userId: user.id,
            approve
        });
 ...
 */
router.delete('/:tagId', auth_1.requireAuth, async (req, res) => {
    try {
        const { user } = req;
        const { tagId } = req.params;
        await photoTaggingService_1.PhotoTaggingService.removeTag(tagId, user.id);
        res.json({
...
router.get('/photo/:photoId', async (req, res) => {
    try {
        ...
        const tags = await photoTaggingService_1.PhotoTaggingService.getPhotoTags(photoId);
        res.json({
            tags,
            count: tags.length
        });
...
router.get('/pending', auth_1.requireAuth, async (req, res) => {
    try {
        const { user } = req;
        const pendingTags = await photoTaggingService_1.PhotoTaggingService.getPendingTags(user.id);
        res.json({
            pendingTags,
 ...
 */
router.get('/search-users', auth_1.requireAuth, async (req, res) => {
    try {
        const { user } = req;
        const { q } = req.query;
        ...
        const users = await photoTaggingService_1.PhotoTaggingService.searchUsersForTagging(q.trim(), user.id);
        res.json({
            users,
            count: users.length
        });
 ...
 */
router.post('/privacy-request', auth_1.requireAuth, async (req, res) => {
    try {
        const { user } = req;
        ...
        if (!Object.values(client_1.PhotoPrivacyRequestType).includes(type)) {
            return res.status(400).json({
                error: 'Invalid request type',
                message: 'Type must be REMOVE_TAG, REMOVE_PHOTO, or BLOCK_FUTURE'
            });
        }
        const request = await photoTaggingService_1.PhotoTaggingService.createPrivacyRequest({
            photoId,
            userId: user.id,
            type,
            reason
...
router.put('/preferences', auth_1.requireAuth, async (req, res) => {
    try {
        ...
        await photoTaggingService_1.PhotoTaggingService.updateTaggingPreferences(user.id, preferences);
        res.json({
...
//# sourceMappingURL=photoTags.js.map