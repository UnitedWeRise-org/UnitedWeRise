...
router.post('/conversations/:conversationId/messages', auth_1.requireAuth, rateLimiting_1.messageLimiter, validation_1.validateMessage, async (req, res) => {
    try {
        ...
        if (otherParticipants.length === 1) {
            ...
            const friendStatus = await relationshipService_1.FriendService.getFriendStatus(userId, recipientId);
            ...
            if (!friendStatus.isFriend) {
                ...
                const existingDeliveredMessage = await prisma_1.prisma.message.findFirst({
                    where: {
                        conversationId,
                        status: 'DELIVERED'
                    }
...
router.get('/requests', auth_1.requireAuth, async (req, res) => {
    try {
        ...
        const pendingMessages = await prisma_1.prisma.message.findMany({
            where: {
                status: 'PENDING',
                conversation: {
                    participants: {
        ...
        logger_1.logger.error({ error, userId: req.user?.id }, 'Get message requests error');
        res.status(500).json({ error: 'Internal server error' });
    }
});
/**
 ...
 *         description: Internal server error
 */
router.put('/requests/:conversationId/accept', auth_1.requireAuth, async (req, res) => {
    try {
        const userId = req.user.id;
        const { conversationId } = req.params;
        ...
        logger_1.logger.error({ error, userId: req.user?.id, conversationId: req.params.conversationId }, 'Accept message request error');
        res.status(500).json({ error: 'Internal server error' });
    }
});
/**
 ...
 *         description: Internal server error
 */
router.put('/requests/:conversationId/decline', auth_1.requireAuth, async (req, res) => {
    try {
        const userId = req.user.id;
        const { conversationId } = req.params;
        // Verify user is a participant in this conversation
        ...
        const remainingMessages = await prisma_1.prisma.message.count({
        ...
        logger_1.logger.error({ error, userId: req.user?.id, conversationId: req.params.conversationId }, 'Decline message request error');
        res.status(500).json({ error: 'Internal server error' });
    }
});
