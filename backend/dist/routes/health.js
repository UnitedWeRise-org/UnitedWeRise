...
function getDeploymentInfo() {
    ...
    try {
        const deployFile = path_1.default.join(__dirname, '../../.deployment-time');
        if (fs_1.default.existsSync(deployFile)) {
            const timestamp = fs_1.default.readFileSync(deployFile, 'utf8').trim();
...
async function getMigrationInfo() {
    ...
    `;
        ...
        logger_1.default.warn('Could not fetch migration info:', error.message);
    }
    return {
        version: 'unknown',
        lastMigration: 'unknown'
...
router.get('/deployment', async (req, res) => {
    try {
        ...
        logger_1.default.error('Deployment status check failed:', error);
        res.status(500).json({
            status: 'error',
            timestamp: new Date().toISOString(),
            error: error.message
...
async function checkAIServices() {
    ...
    try {
        ...
        logger_1.default.warn('AI services check failed:', error);
    }
    return services;
}
...
router.post('/deployment/update', async (req, res) => {
    try {
        ...
        const deployFile = path_1.default.join(__dirname, '../../.deployment-time');
        fs_1.default.writeFileSync(deployFile, timestamp);
        res.json({
            status: 'updated',
            timestamp,
        ...
        logger_1.default.error('Failed to update deployment timestamp:', error);
        res.status(500).json({
            status: 'error',
            error: error.message
        });
