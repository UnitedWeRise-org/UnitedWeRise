 ...
 */
router.get('/organizations', auth_1.requireStagingAuth, auth_1.requireAdmin, async (req, res) => {
    try {
        const { page, limit, offset } = safePagePagination(req.query.page, req.query.limit);
        const search = req.query.search;
        const status = req.query.status || 'all';
        ...
        logger_1.logger.error({ error, adminId: req.user?.id }, 'Failed to list organizations');
 ...
 */
router.get('/organizations/:organizationId', auth_1.requireStagingAuth, auth_1.requireAdmin, async (req, res) => {
    try {
        ...
        const totalMembers = await prisma_1.prisma.organizationMember.count({
            where: { organizationId },
        });
        // Get verification history
        const verificationHistory = await prisma_1.prisma.organizationVerificationRequest.findMany({
            where: { organizationId },
            orderBy: { createdAt: 'desc' },
            select: {
                id: true,
        ...
        logger_1.logger.error({ error, organizationId: req.params.organizationId, adminId: req.user?.id }, 'Failed to get organization details');
 ...
 */
router.post('/organizations/:organizationId/members', auth_1.requireStagingAuth, auth_1.requireAdmin, [
    (0, express_validator_1.body)('userId').isString().notEmpty().withMessage('userId is required'),
    (0, express_validator_1.body)('status').optional().isIn(['PENDING', 'ACTIVE']).withMessage('Invalid status'),
    handleValidationErrors,
], async (req, res) => {
    try {
        // Super-admin check
        ...
        const existingMembership = await prisma_1.prisma.organizationMember.findUnique({
            where: {
        ...
        if (existingMembership) {
            ...
            membership = await prisma_1.prisma.organizationMember.update({
            ...
            membership = await prisma_1.prisma.organizationMember.create({
                data: {
                    organizationId,
                    userId,
                    status: status,
        ...
        // Audit log
        await auditService_1.AuditService.log({
            action: auditService_1.AUDIT_ACTIONS.ADMIN_ORG_MEMBER_ADDED,
            adminId,
        ...
        logger_1.logger.error({ error, organizationId: req.params.organizationId, adminId: req.user?.id }, 'Failed to add member');
 ...
 */
router.delete('/organizations/:organizationId/members/:membershipId', auth_1.requireStagingAuth, auth_1.requireAdmin, async (req, res) => {
    try {
        // Super-admin check
        if (!req.user?.isSuperAdmin) {
            return res.status(403).json({ error: 'Super-admin access required' });
        ...
        // Audit log
        await auditService_1.AuditService.log({
            action: auditService_1.AUDIT_ACTIONS.ADMIN_ORG_MEMBER_REMOVED,
            adminId,
            targetType: 'organization_member',
        ...
        logger_1.logger.error({ error, membershipId: req.params.membershipId, adminId: req.user?.id }, 'Failed to remove member');
 ...
 *         description: Invalid request
 */
router.post('/organizations/:organizationId/transfer-headship', auth_1.requireStagingAuth, auth_1.requireAdmin, [
    (0, express_validator_1.body)('newHeadUserId').isString().notEmpty().withMessage('newHeadUserId is required'),
    ...
    try {
        ...
        const updatedOrg = await organizationService_1.organizationService.transferHeadship(organizationId, newHeadUserId);
        // Get new head info
        const newHead = await prisma_1.prisma.user.findUnique({
            where: { id: newHeadUserId },
            select: { id: true, username: true },
        });
        // Audit log
        await auditService_1.AuditService.log({
            action: auditService_1.AUDIT_ACTIONS.ADMIN_ORG_HEADSHIP_TRANSFERRED,
            adminId,
            targetType: 'organization',
        ...
        logger_1.logger.error({ error, organizationId: req.params.organizationId, adminId: req.user?.id }, 'Failed to transfer headship');
 ...
 *         description: Super-admin access required
 */
router.post('/organizations/:organizationId/deactivate', auth_1.requireStagingAuth, auth_1.requireAdmin, [
    (0, express_validator_1.body)('reason').isString().isLength({ min: 10, max: 500 }).withMessage('Reason must be 10-500 characters'),
    handleValidationErrors,
], async (req, res) => {
    try {
        ...
        // Audit log
        await auditService_1.AuditService.log({
            action: auditService_1.AUDIT_ACTIONS.ADMIN_ORG_DEACTIVATED,
            adminId,
            targetType: 'organization',
            targetId: organizationId,
            details: {
        ...
        logger_1.logger.error({ error, organizationId: req.params.organizationId, adminId: req.user?.id }, 'Failed to deactivate organization');
 ...
 *         description: Super-admin access required
 */
router.post('/organizations/:organizationId/reactivate', auth_1.requireStagingAuth, auth_1.requireAdmin, async (req, res) => {
    try {
        // Super-admin check
        if (!req.user?.isSuperAdmin) {
            return res.status(403).json({ error: 'Super-admin access required' });
        ...
        const headUser = await prisma_1.prisma.user.findUnique({
        ...
        // Audit log
        await auditService_1.AuditService.log({
            action: auditService_1.AUDIT_ACTIONS.ADMIN_ORG_REACTIVATED,
            adminId,
            targetType: 'organization',
            targetId: organizationId,
            details: {
        ...
        logger_1.logger.error({ error, organizationId: req.params.organizationId, adminId: req.user?.id }, 'Failed to reactivate organization');
