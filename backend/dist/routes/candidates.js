...
router.get('/by-level/:level', async (req, res) => {
    try {
        ...
        const offices = await prisma_1.prisma.office.findMany({
            where: {
        ...
        logger_1.logger.error({ error, level: req.params.level }, 'Error fetching offices by level');
        res.status(500).json({
            success: false,
...
router.get('/by-office-type', async (req, res) => {
    try {
        ...
        const normalizedType = officeType.toLowerCase().replace(/\s+/g, '_');
        // Build query
        const whereClause = {
            office: {
                level: level.toUpperCase()
        ...
        logger_1.logger.error({ error, query: req.query }, 'Error fetching candidates by office type');
        res.status(500).json({
            success: false,
 ...
 */
function generateStanceTags(candidate) {
    const tags = [];
    // Extract from key issues
    if (candidate.keyIssues && Array.isArray(candidate.keyIssues)) {
        candidate.keyIssues.slice(0, 3).forEach((issue) => {
            if (issue.length <= 25) {
                tags.push(issue);
            }
            else {
    ...
    if (candidate.policyPositions && candidate.policyPositions.length > 0) {
        candidate.policyPositions.slice(0, 2).forEach((pos) => {
 ...
 */
function getFallbackOfficeTypes(level) {
    const fallbackData = {
        'FEDERAL': [
