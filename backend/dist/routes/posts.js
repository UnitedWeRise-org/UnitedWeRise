 ...
 *         description: Internal server error
 */
router.post('/thread', auth_1.requireAuth, moderation_1.checkUserSuspension, rateLimiting_1.postLimiter, moderation_1.contentFilter, async (req, res) => {
    try {
        ...
        if (Array.isArray(mediaIds))
            photoIds.push(...mediaIds);
        // Generate embedding for head post only (slow operation - 2-3 seconds)
        let embedding = [];
        try {
            const embeddingResult = await azureOpenAIService_1.azureOpenAI.generateEmbedding(headContent.trim());
            embedding = embeddingResult.embedding;
        }
        catch (error) {
            logger_1.logger.warn({ err: error }, 'Failed to generate embedding for thread head');
        }
        // Get user reputation
        const userReputation = await reputationService_1.reputationService.getUserReputation(userId);
        // Get geographic data
        ...
        try {
            await activityTracker_1.ActivityTracker.trackPostCreated(userId, result.headPost.id, headContent.trim());
        }
        catch (error) {
            logger_1.logger.error({ err: error, postId: result.headPost.id }, 'Failed to track thread creation activity');
        }
        ...
        (async () => {
            try {
                ...
                if (subscribersToNotify.length > 0) {
                    ...
                    const notificationPromises = subscribersToNotify.map(sub => (0, notifications_1.createNotification)('NEW_POST', userId, sub.subscriberId, `${authorName} posted a new thread`, result.headPost.id));
                    await Promise.allSettled(notificationPromises);
                    logger_1.logger.info({ postId: result.headPost.id, notifiedCount: subscribersToNotify.length }, 'Thread subscription notifications sent');
                }
            }
            catch (error) {
                logger_1.logger.error({ err: error, postId: result.headPost.id }, 'Failed to send thread subscription notifications');
            }
        })();
        ...
        logger_1.logger.error({ err: error, userId: req.user.id }, 'Thread creation error');
