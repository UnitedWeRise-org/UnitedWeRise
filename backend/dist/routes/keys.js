...
router.post('/upload', auth_1.requireAuth, async (req, res) => {
    try {
        ...
        const key = await prisma_1.prisma.publicKey.upsert({
        ...
        logger_1.logger.info({ userId, deviceId: key.deviceId }, 'Public key uploaded');
        res.json({ success: true, data: key });
    }
    catch (error) {
        logger_1.logger.error({ error, userId: req.user?.id }, 'Error uploading public key');
...
router.get('/:userId', auth_1.requireAuth, async (req, res) => {
    try {
        ...
        const keys = await prisma_1.prisma.publicKey.findMany({
            where: { userId },
        ...
        logger_1.logger.error({ error, targetUserId: req.params.userId }, 'Error fetching public keys');
 ...
 */
router.delete('/:deviceId', auth_1.requireAuth, async (req, res) => {
    try {
        const userId = req.user.id;
        const { deviceId } = req.params;
        const existing = await prisma_1.prisma.publicKey.findUnique({
            where: { userId_deviceId: { userId, deviceId } }
        });
        if (!existing) {
            return res.status(404).json({ success: false, error: 'Public key not found for this device' });
        }
        await prisma_1.prisma.publicKey.delete({
            where: { userId_deviceId: { userId, deviceId } }
        });
        logger_1.logger.info({ userId, deviceId }, 'Public key removed');
        res.json({ success: true, message: 'Public key removed successfully' });
    }
    catch (error) {
        logger_1.logger.error({ error, userId: req.user?.id, deviceId: req.params.deviceId }, 'Error removing public key');
