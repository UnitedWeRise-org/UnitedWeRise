...
export class AuditService {
   ...
   */
  static async log(params: AuditLogParams): Promise<string | null> {
    ...
    try {
      ...
      logger.error({ error, action, adminId, targetType, targetId }, 'Failed to create audit log');
      return null;
    }
  }
   ...
   */
  static async getLogs(options: GetAuditLogsOptions = {}) {
   ...
   */
  static async getActionCounts(since?: Date) {
   ...
   */
  static async getLogsForTarget(targetType: string, targetId: string, limit = 20) {
   ...
   */
  static async getAdminActivity(adminId: string, limit = 50) {
    return prisma.auditLog.findMany({
      where: { adminId },
      orderBy: { createdAt: 'desc' },
      take: limit,
