...
export class RiseAIMentionService {
    ...
    for (const pattern of this.MENTION_PATTERNS) {
      ...
      while ((match = pattern.exec(content)) !== null) {
        ...
        const contentBefore = content.slice(
          Math.max(0, mentionIndex - contextRadius),
          mentionIndex
        ).trim();

        const contentAfter = content.slice(
          mentionIndex + match[0].length,
          mentionIndex + match[0].length + contextRadius
        ).trim();
  ...
  static async getSettings() {
    let settings = await prisma.riseAISettings.findUnique({
      where: { id: 'default' }
    });

    if (!settings) {
      settings = await prisma.riseAISettings.create({
        data: {
          id: 'default',
  ...
  static async checkRateLimit(userId: string): Promise<RateLimitResult> {
    ...
    const usage = await prisma.riseAIUsage.findUnique({
      where: {
        userId_date: {
          userId,
          date: today
  ...
  static async incrementUsage(userId: string): Promise<void> {
    ...
    await prisma.riseAIUsage.upsert({
      where: {
        userId_date: {
          userId,
          date: today
  ...
  static async updateInteraction(
    ...
    return prisma.riseAIInteraction.update({
   ...
   */
  static async getUserInteractions(userId: string, limit: number = 10) {
    return prisma.riseAIInteraction.findMany({
   ...
   */
  static async getInteraction(interactionId: string) {
    return prisma.riseAIInteraction.findUnique({
      where: { id: interactionId },
      include: {
        triggerPost: {
          select: {
  ...
  static async processMention(params: {
    ...
    try {
      ...
      logger.error({ error, params }, 'Failed to process RiseAI mention');
      return {
        success: false,
  ...
  static async updateSettings(
    ...
    return prisma.riseAISettings.update({
      where: { id: 'default' },
      data: {
        ...updates,
        updatedById
