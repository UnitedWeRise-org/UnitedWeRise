...
export class OAuthService {
   ...
   */
  private static hasEncryptionKey(): boolean {
  ...
  private static encryptToken(token: string): string {
    if (!this.hasEncryptionKey()) {
      logger.warn('OAUTH_ENCRYPTION_KEY not set - storing OAuth token unencrypted');
   ...
   */
  private static decryptToken(storedToken: string): string {
    ...
    if (storedToken.startsWith('encrypted:')) {
      if (!this.hasEncryptionKey()) {
        logger.error('Cannot decrypt token - OAUTH_ENCRYPTION_KEY not set');
    ...
    if (!this.hasEncryptionKey()) {
      logger.error('Cannot decrypt legacy token - OAUTH_ENCRYPTION_KEY not set');
