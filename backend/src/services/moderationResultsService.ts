import { PrismaClient, ModerationStatus, ModerationCategory, ModerationDecision } from '@prisma/client';
import { adminDebugLog, adminDebugError } from '../utils/adminDebug.js';
 ...
 */
export class ModerationResultsService {

  /**
   * Create a new moderation result for a photo
   */
  static async createModerationResult(data: CreateModerationResultData) {
    try {
      await adminDebugLog('ModerationResultsService', 'Creating moderation result', { photoId: data.photoId, aiModel: data.aiModel });
      ...
      await adminDebugLog('ModerationResultsService', 'Moderation result created successfully', { resultId: result.id });

      return result;
    } catch (error) {
      await adminDebugError('ModerationResultsService', 'Failed to create moderation result', error);
      throw error;
    }
  }
   ...
   */
  static async getModerationResultByPhotoId(photoId: string) {
    try {
      ...
      await adminDebugError('ModerationResultsService', 'Failed to get moderation result', error);
      throw error;
    }
  }
   ...
   */
  static async queryModerationResults(query: ModerationResultsQuery = {}) {
    try {
      const {
        status,
        requiresReview,
      ...
      await adminDebugLog('ModerationResultsService', 'Queried moderation results', {
      ...
      await adminDebugError('ModerationResultsService', 'Failed to query moderation results', error);
      throw error;
    }
  }
   ...
   */
  static async getPendingReviewItems(limit: number = 20, offset: number = 0) {
    try {
      const results = await prisma.imageModerationResult.findMany({
        where: {
      ...
      await adminDebugError('ModerationResultsService', 'Failed to get pending review items', error);
      throw error;
    }
  }
   ...
   */
  static async createModerationReview(data: CreateModerationReviewData) {
    try {
      await adminDebugLog('ModerationResultsService', 'Creating moderation review', {
      ...
      await adminDebugLog('ModerationResultsService', 'Moderation review created successfully', { reviewId: review.id });

      return review;
    } catch (error) {
      await adminDebugError('ModerationResultsService', 'Failed to create moderation review', error);
      throw error;
    }
  }
  ...
  static async getModerationStatistics() {
    try {
      ...
      await adminDebugError('ModerationResultsService', 'Failed to get moderation statistics', error);
      throw error;
    }
  }
   ...
   */
  private static async updatePhotoModerationStatus(photoId: string, result: any) {
    try {
      ...
      await adminDebugLog('ModerationResultsService', 'Updated photo moderation status', {
        photoId,
        status,
        riskScore: result.riskScore
      });
    } catch (error) {
      await adminDebugError('ModerationResultsService', 'Failed to update photo moderation status', error);
      throw error;
    }
  }
   ...
   */
  private static async applyModerationDecision(photoId: string, decision: ModerationDecision) {
    try {
      ...
      await adminDebugLog('ModerationResultsService', 'Applied moderation decision', {
        photoId,
        decision,
        status
      });
    } catch (error) {
      await adminDebugError('ModerationResultsService', 'Failed to apply moderation decision', error);
      throw error;
    }
  }
   ...
   */
  static async bulkApproveImages(photoIds: string[], reviewerId: string, reason?: string) {
    try {
      await adminDebugLog('ModerationResultsService', 'Bulk approving images', {
      ...
      await adminDebugLog('ModerationResultsService', 'Bulk approval completed', {
        processedCount: results.length
      });

      return results;
    } catch (error) {
      await adminDebugError('ModerationResultsService', 'Failed to bulk approve images', error);
      throw error;
    }
  }
}
