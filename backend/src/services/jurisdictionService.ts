...
export class JurisdictionService {
   ...
   */
  h3ToCoordinates(h3Index: string): { lat: number; lng: number } {
   ...
   */
  getStateCells(stateCode: string, radius = 50): string[] {
    const center = STATE_CENTERS[stateCode.toUpperCase()];
    if (!center) {
      logger.warn({ stateCode }, 'Unknown state code for jurisdiction');
      return [];
    }
   ...
   */
  async getCityCells(city: string, state: string, radius = 10): Promise<string[]> {
   ...
   */
  async getCountyCells(county: string, state: string, radius = 20): Promise<string[]> {
  ...
  getNationalCells(): string[] {
    ...
    for (const [, center] of Object.entries(STATE_CENTERS)) {
      const cell = latLngToCell(center.lat, center.lng, H3_RESOLUTION);
      cells.push(cell);
    }
   ...
   */
  areCoordinatesInJurisdiction(
    lat: number,
    lng: number,
    jurisdictionCells: string[]
  ): boolean {
   ...
   */
  async findOrganizationsForLocation(lat: number, lng: number): Promise<any[]> {
   ...
   */
  async findOrganizationsForH3Cell(h3Cell: string): Promise<any[]> {
    return prisma.organization.findMany({
      where: {
        isActive: true,
        h3Cells: {
  ...
  async findNearbyOrganizations(
    ...
    const nearbyCells = gridDisk(centerCell, rings);
   ...
   */
  async isCandidateInJurisdiction(
    candidateId: string,
