...
class BadgeService {
  ...
  async generateClaimCodes(params: {
    ...
    if (params.type === 'SHARED') {
      ...
      const claimCode = await prisma.badgeClaimCode.create({
        data: {
          code,
   ...
   * @private
   */
  private generateReadableCode(badgeName: string): string {
  ...
  async claimBadgeWithCode(params: {
    userId: string;
    code: string;
  }): Promise<{ success: boolean; userBadge: any; claimRecord: any }> {
    // Find claim code
    const claimCode = await prisma.badgeClaimCode.findUnique({
    ...
    const existingClaim = await prisma.badgeClaim.findUnique({
      where: {
        claimCodeId_userId: {
    ...
    let userBadge;
    try {
      userBadge = await this.awardBadge(
        params.userId,
        claimCode.badgeId,
        undefined,
        `Claimed with code: ${params.code}`
      );
    } catch (error: any) {
    ...
    // Create claim record
    const claimRecord = await prisma.badgeClaim.create({
      data: {
        claimCodeId: claimCode.id,
        userId: params.userId
      }
    ...
    await prisma.badgeClaimCode.update({
  ...
  async awardBadgeBulk(params: {
    ...
    awardedBy: string;
    reason?: string;
  }): Promise<{ awarded: number; failed: number; details: Array<{ email: string; status: string; error?: string }> }> {
    const badge = await prisma.badge.findUnique({
      where: { id: params.badgeId }
    });
    ...
    const details: Array<{ email: string; status: string; error?: string }> = [];
    let awarded = 0;
    let failed = 0;
   ...
   */
  async getClaimCodesByBadge(params: { badgeId: string }): Promise<any[]> {
    return await prisma.badgeClaimCode.findMany({
   ...
   */
  async deactivateClaimCode(params: { claimCodeId: string }): Promise<void> {
    await prisma.badgeClaimCode.update({
