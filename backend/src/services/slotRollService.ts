...
export class SlotRollService {
    ...
    static async generateFeed(
    ...
    ): Promise<{ posts: SlotResult[]; stats: any }> {
        ...
        for (let i = 0; i < cfg.slots; i++) {
            ...
            const post = await this.selectFromPool(
                pool,
                selectedIds,
                { random: randomPool, trending: trendingPool, personalized: personalizedPool },
                userId
            );
    ...
    private static determinePool(
        ...
        if (isLoggedIn) {
            if (roll < config.loggedInThresholds.random) return 'random';
            if (roll < config.loggedInThresholds.trending) return 'trending';
            return 'personalized';
        } else {
            if (roll < config.loggedOutThresholds.random) return 'random';
            return 'trending';
        }
    }
     ...
     */
    private static async selectFromPool(
        pool: PoolType,
        excludeIds: Set<string>,
        pools: { random: any[]; trending: any[]; personalized: any[] },
        userId: string | null
    ): Promise<any | null> {
        const fallbackOrder: PoolType[] = pool === 'personalized'
     ...
     */
    private static weightedRandomSelect(candidates: any[], poolType: PoolType): any {
     ...
     */
    private static async getRandomPool(): Promise<any[]> {
        ...
        const scoredPosts = await Promise.all(posts.map(async (post) => {
            ...
            try {
                const rep = await reputationService.getUserReputation(post.author.id);
                if (rep.current >= 95) reputationMultiplier = 1.1;
                else if (rep.current >= 50) reputationMultiplier = 1.0;
                else if (rep.current >= 30) reputationMultiplier = 0.9;
                else reputationMultiplier = 0.8;
            } catch (e) {
     ...
     */
    private static async getTrendingPool(): Promise<any[]> {
        const thirtyDaysAgo = new Date(Date.now() - 30 * 24 * 60 * 60 * 1000);
...
export const slotRollService = new SlotRollService();
