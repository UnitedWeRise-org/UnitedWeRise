 ...
 */
function getOrganizationId(req: OrgAuthRequest): string | undefined {
  return (
 ...
 */
export const requireOrgCapability = (requiredCapabilities: OrgCapability | OrgCapability[]) => {
  ...
  return async (req: OrgAuthRequest, res: Response, next: NextFunction) => {
    try {
      ...
      if (!userId) {
        logger.warn({ path: req.path }, 'ORG_AUTH: No authenticated user');
        return res.status(401).json({ error: 'Authentication required' });
      }

      const organizationId = getOrganizationId(req);
      if (!organizationId) {
        logger.warn({ path: req.path, userId }, 'ORG_AUTH: No organization ID provided');
      ...
      if (!organization) {
        logger.warn({ organizationId, userId }, 'ORG_AUTH: Organization not found');
        return res.status(404).json({ error: 'Organization not found' });
      }

      if (!organization.isActive) {
        logger.warn({ organizationId, userId }, 'ORG_AUTH: Organization is inactive');
      ...
      if (organization.headUserId === userId) {
        req.orgContext = {
          organizationId,
          isHead: true,
          capabilities: Object.values(OrgCapability),
        };
        return next();
      }
      ...
      if (!membership) {
        logger.info({ organizationId, userId, requiredCapabilities: capabilities }, 'ORG_AUTH: User not a member');
        return res.status(403).json({ error: 'Not a member of this organization' });
      }

      if (membership.status !== MembershipStatus.ACTIVE) {
        logger.info({ organizationId, userId, status: membership.status }, 'ORG_AUTH: Membership not active');
      ...
      logger.error({ error, path: req.path }, 'ORG_AUTH: Error checking capability');
 ...
 */
export const requireOrgMembership = (requireActive = true) => {
  return async (req: OrgAuthRequest, res: Response, next: NextFunction) => {
    try {
      const userId = req.user?.id;
      if (!userId) {
      ...
      if (requireActive && membership.status !== MembershipStatus.ACTIVE) {
        return res.status(403).json({ error: 'Membership is not active' });
      }
      ...
      logger.error({ error, path: req.path }, 'ORG_AUTH: Error checking membership');
      return res.status(500).json({ error: 'Authorization check failed' });
    }
  };
};
...
export const requireOrgHead = () => {
  return async (req: OrgAuthRequest, res: Response, next: NextFunction) => {
    try {
      ...
      if (organization.headUserId !== userId) {
        logger.info({ organizationId, userId }, 'ORG_AUTH: User is not organization head');
      ...
      logger.error({ error, path: req.path }, 'ORG_AUTH: Error checking head status');
      return res.status(500).json({ error: 'Authorization check failed' });
    }
  };
};
 ...
 */
export function hasOrgCapability(req: OrgAuthRequest, capability: OrgCapability): boolean {
  if (!req.orgContext) return false;
  if (req.orgContext.isHead) return true;
