 ...
 *         description: Unauthorized - user not authenticated
 */
router.delete('/:deviceToken', requireAuth, async (req: AuthRequest, res) => {
  try {
    const userId = req.user!.id;
    ...
    logger.error({ error }, 'Failed to unregister device token');
    res.status(500).json({
      success: false,
...
router.get('/', requireAuth, async (req: AuthRequest, res) => {
  try {
    ...
    logger.error({ error }, 'Failed to get device tokens');
    res.status(500).json({
      success: false,
