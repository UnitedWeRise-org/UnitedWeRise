...
router.post('/:candidateId/inquiry', async (req, res) => {
  try {
    ...
    const userId = (req as any).user?.id; // Optional for anonymous inquiries
    const inquiryData = {
      candidateId,
      inquirerId: userId,
      ...req.body
    ...
    const inquiry = await CandidateInboxService.submitInquiry(inquiryData);

    // Track inquiry submissions
    metricsService.incrementCounter('candidate_inquiries_submitted', {
      candidateId,
 ...
 */
router.get('/:candidateId/inbox', requireAuth, async (req: AuthRequest, res) => {
  try {
    const { candidateId } = req.params;
    ...
    // Track inbox access
    metricsService.incrementCounter('candidate_inbox_accessed', {
      candidateId,
      userId
    });
 ...
 */
router.post('/inquiry/:inquiryId/respond', requireAuth, async (req: AuthRequest, res) => {
  try {
    ...
    const response = await CandidateInboxService.respondToInquiry(responseData);

    // Track responses
    metricsService.incrementCounter('candidate_inquiry_responses', {
      candidateId: inquiry.candidateId,
...
router.get('/:candidateId/public-qa', async (req, res) => {
  try {
    ...
    metricsService.incrementCounter('candidate_public_qa_views', {
      candidateId
    });
 ...
 */
router.post('/:candidateId/public-qa/:qaId/vote', requireAuth, async (req: AuthRequest, res) => {
  try {
    ...
    // Track votes
    metricsService.incrementCounter('candidate_qa_votes', {
      candidateId,
 ...
 *         description: Candidate not found
 */
router.post('/:candidateId/staff', requireAuth, async (req: AuthRequest, res) => {
  try {
    const { candidateId } = req.params;
    const userId = req.user!.id;
    ...
    const staffMember = await CandidateInboxService.addStaffMember(candidateId, staffUserId, {
      role,
      permissions,
      addedBy: userId
    });
    ...
    metricsService.incrementCounter('candidate_staff_added', {
      candidateId,
 ...
 *         description: Candidate not found
 */
router.get('/:candidateId/staff', requireAuth, async (req: AuthRequest, res) => {
  try {
    const { candidateId } = req.params;
    const userId = req.user!.id;
