 ...
 */
function validateFallbackProof(
  fallbackProof: any,
  ...
  if (!fallbackProof.timingSeconds || fallbackProof.timingSeconds < 3) {
    req.log.warn({ ip: req.ip, timing: fallbackProof.timingSeconds },
      'Fallback registration rejected: form submitted too quickly');
  ...
  if (riskScore >= 30) {
    req.log.warn({ ip: req.ip, riskScore },
      'Fallback registration rejected: high risk score');
