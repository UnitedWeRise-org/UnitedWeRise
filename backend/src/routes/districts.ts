...
const CONGRESSIONAL_DISTRICTS_BY_STATE: Record<string, number> = {
...
const STATE_SENATE_DISTRICTS_BY_STATE: Record<string, number> = {
...
const STATE_HOUSE_DISTRICTS_BY_STATE: Record<string, number> = {
...
router.get('/', async (req: Request, res: Response) => {
  try {
    ...
    let maxDistricts = 0;
    let labelPrefix = '';
    let identifierFormat: (num: number) => string;
    ...
    try {
      const existingDistricts = await prisma.electoralDistrict.findMany({
        where: {
          type: type as any,
          state: stateCode,
        },
      ...
      logger.debug({ error: dbError, state: stateCode, type }, 'Could not fetch existing districts');
    }
    ...
    logger.error({ error }, 'Failed to fetch districts');
