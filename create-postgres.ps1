...
$PG_RESULT = az postgres flexible-server create --resource-group unitedwerise-rg --name unitedwerise-db --location centralus --admin-user uwradmin --admin-password "UWR-Secure2024!" --sku-name Standard_B1ms --tier Burstable --storage-size 32 --version 14 --public-access 0.0.0.0-255.255.255.255 --yes
...
if ($LASTEXITCODE -eq 0) {
    ...
    $INSIGHTS_KEY = az monitor app-insights component show --resource-group unitedwerise-rg --app unitedwerise-insights --query connectionString -o tsv
...
"@
    ...
    Write-Host "Next: We'll deploy your application to Container Apps" -ForegroundColor Cyan
    
} else {
    Write-Host "`nCentral US also restricted. Trying North Europe..." -ForegroundColor Yellow
    
    # Try North Europe as last resort
    az postgres flexible-server create --resource-group unitedwerise-rg --name unitedwerise-db --location northeurope --admin-user uwradmin --admin-password "UWR-Secure2024!" --sku-name Standard_B1ms --tier Burstable --storage-size 32 --version 14 --public-access 0.0.0.0-255.255.255.255 --yes
