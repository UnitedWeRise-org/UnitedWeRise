...
$PG_RESULT = az postgres flexible-server create --resource-group unitedwerise-rg --name unitedwerise-db --location centralus --admin-user uwradmin --admin-password "$env:DB_ADMIN_PASSWORD" --sku-name Standard_B1ms --tier Burstable --storage-size 32 --version 14 --public-access 0.0.0.0-255.255.255.255 --yes
